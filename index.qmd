...
This simulation-based approach mirrors a lot of the computational thinking behind Bayesian statistics. In both approaches, we:

- Start with an explicit data-generating process
- Think about uncertainty with simulation instead of formulas
- Compare observed data to a distribution
...
If that sounds fun—and if you'd rather say things like "there's an X% probability that $\delta$ is positive" or "$\delta$ is most likely between X and Y" instead of the convoluted "there's an X% probability of seeing a $\delta$ at least this extreme in a null world"—check out [*Bayes Rules!*](https://www.bayesrulesbook.com/) for the best introduction I've found to Bayesian modeling.
